...
- **Context**: Adding a P0 hotfix task to a sub-project's task_db before launching that sub-project's `scripts/autonomous_run.sh`. Calling `python "$ROOT/<subproject>/.claude/hooks/task_db.py" add --priority P0 ...` from a Bash session whose cwd is the workspace root returns `{"ok": true, "id": "T-P0-N"}` -- but the row lands in the WRONG db.
- **What went wrong**: The autonomous_run.sh's inner Claude session reported "no unblocked tasks" and worked on stale filler instead. Direct sqlite probe confirmed the row sat in the **root** `.claude/tasks.db`, not the sub-project's. Reason: `task_db.py`'s `_find_project_root()` returns the first ancestor of `Path.cwd()` that contains a `CLAUDE.md`. Both root and helixos have CLAUDE.md. With cwd = root, the script picked root's DB. The script's binary location (`<subproject>/.claude/hooks/`) is irrelevant -- `__file__` is only the fallback when cwd has no CLAUDE.md.
...
- **Related task**: root T-P0-253 (mis-routed) -> root T-P0-626 (re-added correctly after delete from root); root T-P1-255 (--project flag), root T-P2-259 (propagation).
...
- **Note**: This sharpens the existing 2026-04-13 [PROPAGATED] "Orchestrator `all_done` flag is sticky" lesson above. The 2026-04-13 entry covered the orchestrator loop bailing on stale `all_done`; this entry covers the inner Claude session also reading `session_state.json` and treating `all_done=true` as authoritative even before the orchestrator-loop check fires.
- **Context**: After (mis-)adding a P0 hotfix to a sub-project's task_db, launched `bash <subproject>/scripts/autonomous_run.sh`. Inner Claude session opened, looked at session_state.json (which had `all_done=true` from a prior run that legitimately drained the queue), and concluded there was nothing to do. Picked up filler work (emoji-cleanup of unrelated scripts) instead of re-checking task_db.
- **What went wrong**: session_state.json is sticky across orchestrator invocations at TWO layers: the orchestrator loop (covered by 2026-04-13 entry) AND the inner Claude session's "what should I work on" decision. The inner session inherits the prior state and treats it as authoritative even when task_db.py reports unblocked work. Same sticky-gate class as the 2026-04-19 [UNIVERSAL] "human-approval gate prose stays sticky" lesson, but in orchestrator persistent state.
...
- **Related task**: root T-P0-626 (the hotfix that autonomous_run.sh failed to pick up); root T-P1-257 (orchestrator startup reset), root T-P2-259 (propagation).
